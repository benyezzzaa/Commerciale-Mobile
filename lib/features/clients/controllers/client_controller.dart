...
class ClientController extends GetxController {
    ...
    try {
      ...
      if (!_validateClientData(cleanData)) {
        Get.snackbar(
          ...
          'Vérifiez que tous les champs sont correctement remplis (SIRET: 14 chiffres, téléphone: format français)',
  ...
  Future<ClientModel?> updateClient({
    ...
    try {
      final response = await dio.put(
        '${AppApi.getClientsUrl}/$id',
        data: cleanData,
        options: Options(headers: {'Authorization': 'Bearer $token'}),
      );
      final updatedClient = ClientModel.fromJson(response.data);
  ...
  bool _validateClientData(Map<String, dynamic> data) {
    try {
      ...
      // Validation spécifique du téléphone français (avec espaces)
      ...
      if (!RegExp(r'^(\+33|0|33)[1-9](\d{8})$').hasMatch(cleanTelephone)) {
        ...
        print('   Format attendu: 06 12 34 56 78, +33 6 12 34 56 78 ou 33 6 12 34 56 78');
        return false;
      }
      ...
      if (!RegExp(r'^\d{14}$').hasMatch(codeFiscale)) {
        print('❌ SIRET invalide: $codeFiscale (doit contenir 14 chiffres)');
        return false;
      }
  ...
  /// Formatage automatique du téléphone français (avec espaces)
  String _formatTelephone(String telephone) {
    // Nettoyer les espaces, tirets et points
    final cleanNumber = telephone.replaceAll(RegExp(r'[\s\-\.]'), '').replaceAll(RegExp(r'[^\d+]'), '');
